...
function initVoice() {
  ...
  voiceBtn.addEventListener('click', () => {
    ...
    transcriptEl.innerHTML = '<div class="mic-dot"></div><span>Habla ahora... Ej: "Vendí 3 café volcán en efectivo"</span>';
    recognition.start();
  });
  ...
  function stopListening() {
    ...
    setTimeout(() => transcriptEl.classList.remove('visible'), 3000);
  }
}

// ─── Parser de comando de voz ─────────────────────────────────
function parseVoiceCommand(text) {
  // "vendí 3 café volcán en efectivo"
  // "vendí/vendi [cantidad] [producto] en [pago]"
  const pagoMap = {
    efectivo: 'efectivo', contado: 'efectivo',
    crédito: 'credito', credito: 'credito',
    transferencia: 'transferencia', transferir: 'transferencia', nequi: 'transferencia', daviplata: 'transferencia',
    tarjeta: 'tarjeta',
  };

  const pagoMatch = Object.keys(pagoMap).find(k => text.includes(k));
  const pago = pagoMap[pagoMatch] || 'efectivo';

  const numMatch = text.match(/\b(\d+)\b/);
  const cantidad = numMatch ? parseInt(numMatch[1]) : 1;

  // Extraer producto: quitar "vendí X ... en pago"
  let producto = text
    .replace(/vend[ií]\s*/i, '')
    .replace(new RegExp(`\\b${numMatch?.[0]}\\b`), '')
    .replace(/\ben\b.*$/i, '')
    .replace(/\b(efectivo|contado|crédito|credito|transferencia|tarjeta|nequi|daviplata)\b/gi, '')
    .trim()
    .replace(/\s+/g, ' ');

  if (!producto) { toast('No entendí el producto. Intenta de nuevo.', 'warning', '⚠️'); return; }

  // Rellenar form
  document.getElementById('ing-producto').value = capitalize(producto);
  document.getElementById('ing-cantidad').value = cantidad;

  // Seleccionar forma de pago
  document.querySelectorAll('#form-ingresos .payment-btn').forEach(b => {
    b.classList.toggle('selected', b.dataset.val === pago);
  });

  toast(`Comando interpretado: ${cantidad}× ${capitalize(producto)} (${pago})`, 'success', '🎤');
}
